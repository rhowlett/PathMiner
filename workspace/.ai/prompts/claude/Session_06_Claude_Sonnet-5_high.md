...
- **Parallel wave:** W3
- **Prerequisite sessions:** Session 05
...
Define BoardSource, implement FileBoardSource, and provide stable REF.PAD lookup plus drift diagnostics.
...
1. Work in a dedicated branch/worktree named `ai/session-06-claude-board-source`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 05.
...
- pathminer/kicad/board.py
- pathminer/kicad/source.py
- pathminer/models/board.py
- tests/test_board_source.py
- tests/fixtures/kicad/
...
- BoardSource protocol
- FileBoardSource adapter
- stable terminal lookup and named drift failures

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_board_source.py
...
- `.ai/handoffs/Session_06_Claude.md`
- `.ai/handoffs/Session_06_Claude.json`
