# Session 18 — Analysis configuration and path unification
...
- **Parallel wave:** W11-B
- **Prerequisite sessions:** Session 13, Session 16, Session 17
...
Build Analysis configuration/run/manual-estimate views and merge Trace and Via/Path into one consistent workflow.
...
- PWR-002 (override/explanation slice)
...
1. Work in a dedicated branch/worktree named `ai/session-18-claude-ui-analysis`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 13, Session 16, Session 17.
...
- pathminer/ui/tabs/analysis.py
- pathminer/ui/widgets/path_editor.py
- pathminer/ui/widgets/override_editor.py
- tests/ui/test_analysis.py
- tests/ui/test_path_editor.py
...
- one-segment and multi-segment path workflow
- representative-versus-equivalent distinction
- inherited overrides with provenance
- model choice and override control

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/ui/test_analysis.py tests/ui/test_path_editor.py
...
- `.ai/handoffs/Session_18_Claude.md`
- `.ai/handoffs/Session_18_Claude.json`
