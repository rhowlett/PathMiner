# Session 22 — Component sidecar, snapshots and captured paths
...
- **Parallel wave:** W15
- **Prerequisite sessions:** Session 21
...
Implement reproducible component models and captured multi-net paths with strict bridge resolution.
...
1. Work in a dedicated branch/worktree named `ai/session-22-claude-component-path-models`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 21.
...
- pathminer/models/components.py
- pathminer/models/path.py
- pathminer/storage/component_models.py
- pathminer/analysis/path_resolution.py
- schema/component_model.schema.json
- schema/captured_path.schema.json
...
- lean sidecar and precedence
- project-local resolved snapshots
- REF.PAD hop resolution
- complete missing-bridge diagnostics

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_component_models.py tests/test_path_resolution.py
...
- `.ai/handoffs/Session_22_Claude.md`
- `.ai/handoffs/Session_22_Claude.json`
