# Session 25 — Verdicts, margins and parallel branches
...
- **Parallel wave:** W17
- **Prerequisite sessions:** Session 24
...
Implement multi-reason verdicts, causal budgets/margins, and nodal parallel branch current sharing.
...
1. Work in a dedicated branch/worktree named `ai/session-25-claude-power-verdicts-parallel`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 24.
...
- pathminer/analysis/verdicts.py
- pathminer/analysis/margins.py
- pathminer/analysis/parallel.py
- tests/test_verdicts.py
- tests/test_margins.py
- tests/test_parallel_paths.py
...
- ordered multi-reason verdict
- source/copper/bridge budget
- N-branch imbalance calculation
- analytic parallel tests

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_verdicts.py tests/test_margins.py tests/test_parallel_paths.py
...
- `.ai/handoffs/Session_25_Claude.md`
- `.ai/handoffs/Session_25_Claude.json`
