# Session 20 — Reports, Diagnostics and diagnostic bundles
...
- **Parallel wave:** W13
- **Prerequisite sessions:** Session 14, Session 16, Session 17, Session 19
...
Make Reports a pure immutable-run consumer and centralize validation, jobs, logs, solver diagnostics, and support bundles.
...
- QA-007 (GUI test implementation)
- BASE-003 (report/help naming slice)
...
1. Work in a dedicated branch/worktree named `ai/session-20-claude-ui-reports-diagnostics`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 14, Session 16, Session 17, Session 19.
...
- pathminer/ui/tabs/reports.py
- pathminer/ui/tabs/diagnostics.py
- pathminer/storage/diagnostic_bundle.py
...
- pure-consumer Reports destination
- central Diagnostics destination
- sanitized diagnostic bundle
- GUI state/accessibility tests

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/ui/test_reports.py tests/ui/test_diagnostics.py tests/test_diagnostic_bundle.py
...
- `.ai/handoffs/Session_20_Claude.md`
- `.ai/handoffs/Session_20_Claude.json`
