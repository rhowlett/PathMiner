# Session 33 — Complex signal-return network and coupling
...
- **Parallel wave:** W22-B
- **Prerequisite sessions:** Session 32
...
Implement frequency-aware R/L/C/via/capacitor impedance and a validated coupling/PEEC or external-solver method.
...
- QA-001 (AC impedance equation slice)
...
1. Work in a dedicated branch/worktree named `ai/session-33-claude-signal-complex`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 32.
...
- pathminer/analysis/complex_network.py
- pathminer/analysis/signal_return.py
- pathminer/models/impedance.py
- tests/test_complex_network.py
- tests/test_signal_return.py
...
- complex impedance edges
- complex nodal conservation
- coupling/correlation adapter
- analytic R/L/C vectors

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_complex_network.py tests/test_signal_return.py
...
- `.ai/handoffs/Session_33_Claude.md`
- `.ai/handoffs/Session_33_Claude.json`
