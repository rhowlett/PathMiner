# Session 28 — System-power release gate
...
- **Parallel wave:** W20
- **Prerequisite sessions:** Session 23, Session 27
...
Integrate and verify the system-power capability without weakening the Resistance v1 contract.

## Punch-list responsibility

Closure owner in this session:

- No whole-item closure; this session delivers a named capability slice.

Contributes to or extends these living/cross-session items:

- QA-003 (system-power workflow slice)
- QA-004 (power-board slice)
- QA-006 (power budgets)
- QA-008 (system-power confidence slice)
- QA-009 (system-power gate slice)
...
1. Work in a dedicated branch/worktree named `ai/session-28-claude-power-v1-gate`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 23, Session 27.
...
- system-power acceptance suite
- published capability limits
- performance and convergence budgets
...
- python3 -m pytest -q tests/integration/test_system_power.py tests/performance/test_system_power.py
...
- `.ai/handoffs/Session_28_Claude.md`
- `.ai/handoffs/Session_28_Claude.json`
