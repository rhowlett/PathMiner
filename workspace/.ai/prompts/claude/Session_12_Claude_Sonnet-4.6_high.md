# Session 12 — Immutable runs and portable analysis bundles
...
- **Parallel wave:** W9-A
- **Prerequisite sessions:** Session 11
...
Create immutable run manifests plus checksum-validated analysis bundle export/import.
...
1. Work in a dedicated branch/worktree named `ai/session-12-claude-runs-bundles`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 11.
...
- pathminer/storage/runs.py
- pathminer/storage/bundle.py
...
- immutable run IDs and manifests
- portable bundle format
- checksum and path-sanitization tests

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_run_manifest.py tests/test_bundle.py
...
- `.ai/handoffs/Session_12_Claude.md`
- `.ai/handoffs/Session_12_Claude.json`
