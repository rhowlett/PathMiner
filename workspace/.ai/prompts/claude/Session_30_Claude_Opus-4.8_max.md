# Session 30 — DC return metrics, maps and release evidence
...
- **Parallel wave:** W22-A
- **Prerequisite sessions:** Session 29
...
Compute trace/via/mesh current density, voltage gradients, and conserved Joule-loss maps with no unsupported temperature claim.
...
- QA-001 (return equation slice)
- QA-004 (return-board slice)
- QA-006 (return budgets)
- QA-008 (DC-return confidence slice)
- QA-009 (DC-return gate slice)
...
1. Work in a dedicated branch/worktree named `ai/session-30-claude-dc-return-maps`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 29.
...
- pathminer/analysis/current_density.py
- pathminer/analysis/loss_map.py
- pathminer/report/return_maps.py
- tests/test_current_density.py
- tests/test_loss_map.py
- tests/integration/test_dc_return.py
- documents/model_confidence.md
- documents/release_gates.md
...
- unit-explicit current density
- watts/cell and W/mm2 heat-source maps
- current/power conservation evidence
- DC-return release slice

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_current_density.py tests/test_loss_map.py tests/integration/test_dc_return.py
...
- `.ai/handoffs/Session_30_Claude.md`
- `.ai/handoffs/Session_30_Claude.json`
