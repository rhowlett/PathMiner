# Session 17 — Investigation, shared scope and explicit pairs
...
- **Parallel wave:** W9-B
- **Prerequisite sessions:** Session 16
...
Build searchable board investigation and replace all net/pair pickers with shared, persistent selection controls.
...
1. Work in a dedicated branch/worktree named `ai/session-17-claude-ui-investigation`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 16.
...
- pathminer/ui/tabs/investigation.py
- pathminer/ui/widgets/scope_picker.py
- pathminer/ui/widgets/pair_editor.py
- tests/ui/test_investigation.py
- tests/ui/test_scope_picker.py
...
- searchable net/refdes/pad browser
- one SelectionModel-backed scope picker
- validated explicit pairs
- keyboard and non-color behavior

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/ui/test_investigation.py tests/ui/test_scope_picker.py tests/ui/test_accessibility.py
...
- `.ai/handoffs/Session_17_Claude.md`
- `.ai/handoffs/Session_17_Claude.json`
