# Session 31 — Thermal, electrothermal and correlation gate
...
- **Parallel wave:** W23-A
- **Prerequisite sessions:** Session 30
...
Preserve scoped IPC estimates, add a board thermal network and electrothermal iteration, and publish validation limits.
...
- QA-001 (thermal equation slice)
- QA-006 (thermal budgets)
- QA-008 (thermal confidence slice)
- QA-009 (thermal gate slice)
...
1. Work in a dedicated branch/worktree named `ai/session-31-claude-thermal`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 30.
...
- pathminer/analysis/thermal.py
- pathminer/analysis/electrothermal.py
- pathminer/report/thermal.py
- tests/test_thermal.py
- tests/test_electrothermal.py
- tests/correlation/thermal/
...
- scoped IPC estimator
- board thermal network
- reported electrothermal convergence
- analytic/refinement/external correlation evidence

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_thermal.py tests/test_electrothermal.py tests/correlation/thermal
...
- `.ai/handoffs/Session_31_Claude.md`
- `.ai/handoffs/Session_31_Claude.json`
