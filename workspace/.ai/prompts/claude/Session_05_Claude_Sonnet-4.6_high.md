...
- **Parallel wave:** W2-C
- **Prerequisite sessions:** Session 02
...
Extract the KiCad S-expression, stackup, and preferences readers into isolated modules without changing the monolith.
...
1. Work in a dedicated branch/worktree named `ai/session-05-claude-kicad-parser`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 02.
...
- pathminer/kicad/sexpr.py
- pathminer/kicad/stackup.py
- pathminer/kicad/prefs.py
- tests/test_kicad_sexpr.py
...
- pure KiCad readers
- fixtures for malformed and supported input
- stackup parity record

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_kicad_sexpr.py tests/test_stackup.py tests/test_prefs.py
...
- `.ai/handoffs/Session_05_Claude.md`
- `.ai/handoffs/Session_05_Claude.json`
