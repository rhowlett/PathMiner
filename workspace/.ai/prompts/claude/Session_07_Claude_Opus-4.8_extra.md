...
- **Parallel wave:** W4
- **Prerequisite sessions:** Session 03, Session 04, Session 06
...
Create the common resistor-network model and pure point-to-point, ladder, mesh, and automatic-selection builders.
...
- ARCH-006 (dispatcher prerequisites)
- PWR-002 (policy slice)
- QA-005 (correlation fixture slice)
...
1. Work in a dedicated branch/worktree named `ai/session-07-claude-network-builders`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 03, Session 04, Session 06.
...
- pathminer/core/network.py
- pathminer/analysis/builders/
- pathminer/analysis/model_selection.py
...
- typed common network
- three builders
- explainable automatic-selection policy
- fast-versus-mesh fixtures

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_network.py tests/test_builders.py tests/test_model_selection.py
...
- `.ai/handoffs/Session_07_Claude.md`
- `.ai/handoffs/Session_07_Claude.json`
