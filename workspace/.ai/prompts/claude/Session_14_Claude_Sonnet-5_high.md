# Session 14 — Report registry and canonical result renderers
...
Create a plugin-like report registry and render canonical immutable results to JSON, text, Markdown, and PDF.
...
1. Work in a dedicated branch/worktree named `ai/session-14-claude-report-registry`.
...
- schema/report_options.schema.json
...
- report registration contract
- canonical JSON output
- immutable run selection
- renderer equality tests

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_report_registry.py tests/test_renderers.py
...
- `.ai/handoffs/Session_14_Claude.md`
- `.ai/handoffs/Session_14_Claude.json`
