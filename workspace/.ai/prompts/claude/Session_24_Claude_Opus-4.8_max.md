# Session 24 — Source, load and current-limit physics
...
- **Parallel wave:** W16-B
- **Prerequisite sessions:** Session 22
...
Implement source OCV/resistance/current limits and current, resistance, and power load operating points.
...
1. Work in a dedicated branch/worktree named `ai/session-24-claude-source-load-physics`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 22.
...
- pathminer/models/power.py
- pathminer/analysis/operating_point.py
- tests/test_source_models.py
- tests/test_load_models.py
- tests/test_current_limits.py
...
- conditioned source models
- three load models
- uniform post-solve current-limit regime
- analytic/no-steady-state vectors

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_source_models.py tests/test_load_models.py tests/test_current_limits.py
...
- `.ai/handoffs/Session_24_Claude.md`
- `.ai/handoffs/Session_24_Claude.json`
