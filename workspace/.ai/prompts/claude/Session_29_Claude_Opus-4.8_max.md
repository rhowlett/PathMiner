# Session 29 — DC return geometry, mesh, injections and solve
...
Create the resistive return mesh and solve balanced simultaneous DC return injections while excluding ideal capacitive DC action.
...
1. Work in a dedicated branch/worktree named `ai/session-29-claude-dc-return-solver`.
...
- pathminer/analysis/dc_return.py
- pathminer/analysis/return_mesh.py
- pathminer/models/return_path.py
- tests/test_return_mesh.py
- tests/test_dc_return.py
...
- return terminal injections
- obstacle-aware plane mesh
- explicit leakage-only capacitor DC behavior
- KCL-balanced scenario solve

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_return_mesh.py tests/test_dc_return.py
...
- `.ai/handoffs/Session_29_Claude.md`
- `.ai/handoffs/Session_29_Claude.json`
