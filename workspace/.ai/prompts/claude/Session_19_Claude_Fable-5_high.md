# Session 19 — Reusable results and compare/what-if UI
...
- **Parallel wave:** W12
- **Prerequisite sessions:** Session 18
...
Create the reusable verdict/result renderer and traceable comparison UI used by Analysis and report preview.
...
1. Work in a dedicated branch/worktree named `ai/session-19-claude-ui-results-compare`.
2. Start from the exact coordinator-approved commit containing all prerequisites: Session 18.
...
- pathminer/ui/widgets/result_renderer.py
- pathminer/ui/widgets/result_compare.py
- tests/ui/test_result_renderer.py
- tests/ui/test_result_compare.py
...
- verdict/margin/budget/assumption sections
- baseline-versus-variant comparison shell

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/ui/test_result_renderer.py tests/ui/test_result_compare.py
...
- `.ai/handoffs/Session_19_Claude.md`
- `.ai/handoffs/Session_19_Claude.json`
