# Session 03 — Units, materials, trace and via extraction
...
Extract pure unit, material, trace, and via calculations with stable numerical vectors; do not edit the legacy monolith.
...
- ARCH-010 (module-vector slice)
- QA-001 (DC resistance equation slice)
...
1. Work in a dedicated branch/worktree named `ai/session-03-claude-electrical-core`.
...
- ported stable vector IDs
- no Qt/KiCad/I/O imports

## Verification

Run these commands when the named files exist, plus any narrower tests needed for changed code:

- python3 -m pytest -q tests/test_units.py tests/test_materials.py tests/test_resistance.py
...
- `.ai/handoffs/Session_03_Claude.md`
- `.ai/handoffs/Session_03_Claude.json`
